...
class DropboxClient
    ...
    # A way of keeping up with the state of the Dropbox folder hierarchy.  You can
    # periodically call delta() to get a list of "delta entries", which are
    # instructions on how to update your local state to match the server's
    # state.
    #
    # Arguments:
    # * cursor: On the first call, you can omit this argument or pass
    #   in nil.  On subsequent calls, pass in the 'cursor' string
    #   returned by the previous call.
    #
    # Returns: A hash with three fields.
    # * delta: A list of "delta entries" (described below)
    # * cursor: A cursor that indicates the state your local view of
    #   the Dropbox after applying the delta entries.  On the next
    #   call to #delta_beta(), pass in this value.
    # * has_more: If there are more delta entries available to retrieve
    #   right now.
    #
    # Delta Entries: Each entry is a list of one of following forms:
    # * <tt>['add', </tt><em>metadata</em><tt>]</tt>: Indicates that the file/folder specified
    #   by <em>metadata</em> should be added to your local state.  The <em>metadata</em>
    #   value is the same as the one returned by the #metadata() call.
    #   * If the path refers to parent folders that don't yet exist,
    #     those parent folder should be created automatically.
    #   * If an file or empty folder already exists at the given path, it
    #     should be replaced with the new entry.
    #   * If <em>metadata</em> is for a new folder, and its path refers to an
    #     existing non-empty folder, just apply the new metadata to the
    #     existing folder.  Do not modify any children of the folder.
    # * <tt>['del', </tt><em>path</em><tt>]</tt>: Indicates that the file or folder at <em>path</em>
    #   should be deleted.  If there is a folder at the given path, all
    #   its children should also be deleted.  If there is nothing at the
